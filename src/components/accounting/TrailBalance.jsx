...
const TrialBalance = () => {
    ...
    const printTrialBalance = () => {
        ...
        printWindow.document.write(`
                ...
                <body>
                    ...
                    <table>
                        ...
                        <tbody>
                            ${data.rows.map(row => {
            const balance = row.debit - row.credit;
            const rowClass = row.ledger.includes('Party') ? 'party-row' :
                row.ledger.includes('Truck') ? 'truck-row' : 'income-row';
            ...
            return `
                    ...
                    <div class="footer">
                        <p>Generated by Accounting System | Total Accounts: ${data.rows.length}</p>
                    </div>
                </body>
            </html>
        `);
    ...
    const getFilteredData = () => {
        if (!data || filterType === 'all') return data?.rows || [];

        return data.rows.filter(row => {
            if (filterType === 'party') return row.ledger.includes('Party');
            if (filterType === 'truck') return row.ledger.includes('Truck');
    ...
    const getChartData = () => {
        ...
        return filteredRows.map(row => ({
            ...
            fill: row.ledger.includes('Party') ? '#3b82f6' :
                row.ledger.includes('Truck') ? '#f59e0b' :
                    '#10b981'
        }));
    };
    ...
    const getLedgerIcon = (ledger) => {
        if (ledger.includes('Party')) return <User className="w-4 h-4" />;
        if (ledger.includes('Truck')) return <Truck className="w-4 h-4" />;
        if (ledger.includes('Commission')) return <Percent className="w-4 h-4" />;
        return <CreditCard className="w-4 h-4" />;
    };

    // Calculate category totals
    const calculateCategoryTotals = () => {
        if (!data) return { party: 0, truck: 0, income: 0 };

        const partyRows = data.rows.filter(row => row.ledger.includes('Party'));
        const truckRows = data.rows.filter(row => row.ledger.includes('Truck'));
        const incomeRows = data.rows.filter(row => row.ledger.includes('Commission'));

        return {
            party: partyRows.reduce((sum, row) => sum + Math.abs(row.debit - row.credit), 0),
            truck: truckRows.reduce((sum, row) => sum + Math.abs(row.debit - row.credit), 0),
            income: incomeRows.reduce((sum, row) => sum + Math.abs(row.debit - row.credit), 0)
        };
    };
                ...
                <div className="bg-white p-5 rounded-xl border border-gray-200 shadow-sm hover:shadow-md transition-shadow">
                    ...
                    <div className="mt-3 pt-3 border-t border-gray-100">
                        <div className="flex justify-between text-xs text-gray-500">
                            <span>Parties: {data ? data.rows.filter(r => r.ledger.includes('Party')).length : 0}</span>
                            <span>Trucks: {data ? data.rows.filter(r => r.ledger.includes('Truck')).length : 0}</span>
                        </div>
                    </div>
                </div>
            </div>
                    ...
                    <div className="flex justify-between items-center">
                        ...
                        <div className="flex items-center space-x-2">
                            <span className="text-sm text-gray-600">
                                Showing {filteredRows.length} of {data?.rows.length || 0} accounts
                            </span>
                            <div className="flex bg-gray-100 rounded-lg p-1">
                                <button
                            ...
                            <tbody className="divide-y divide-gray-200">
                                {loading ? (
                                    ...
                                    filteredRows.map((row, index) => {
                                        const balance = row.debit - row.credit;
                                        const ledgerType = row.ledger.includes('Party') ? 'Party' :
                                            row.ledger.includes('Truck') ? 'Truck' : 'Income';
